...
def test_polygon():
    ...
    assert math.isclose(
        p.side_length, math.sqrt(2), rel_tol=rel_tol, abs_tol=abs_tol
    ), (f"actual: {p.side_length}," f" expected: {math.sqrt(2)}")
...
def test_polygons():
    ...
    polygons_list = []
    for p in polygons:
        polygons_list.append(p)
    assert len(polygons_list) == 3, f"actual: {len(polygons_list)}, expected: 3"
    for p in polygons:
        polygons_list.append(p)
    assert len(polygons_list) == 6, f"actual: {len(polygons_list)}, expected: 3"
    assert polygons.max_efficiency_polygon == Polygon(5, 3)
